...
export async function translate(query: TranslateQuery, entrypoint: string, apiKey: string) {
  ...
  switch (query.mode) {
            ...
            答案应该使用中文。`
                    ...
                    assistantPrompt = `Please answer in markdown format with two section 'Maybe' and 'Desc'. 'Maybe' should provide the most likely thing it is (be more precise), while 'Desc' should describe what this thing is. And you answer must be ${lang.langMap.get(query.detectTo) || query.detectTo}.`
                  }
                  break
                case 'polishing':
                  systemPrompt =
